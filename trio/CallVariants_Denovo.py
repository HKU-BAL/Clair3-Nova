...
def batch_output(batch_chr_pos_seq, alt_info_list, batch_Y, output_config, output_utilities):
    ...
    if output_config.is_denovo:
        _tar_cum = [0] + param.label_shape_cum_trio_denovo
    ...
    batch_denovo = [0] * batch_size
    if output_config.is_denovo:
        # last two index are for denovo
        batch_denovo = batch_Y[: , _tar_cum[-2]:]
...
def output_with(
    ...
    if output_config.is_debug:
        ...
        if output_config.gvcf:
            ...
            output_utilities.output("%s\t%d\t.\t%s\t%s\t%.2f\t%s\t%s%s\tGT:GQ:DP:AD:AF\t%s:%d:%d:%s:%s" % (
                    chromosome,
                    position,
                    reference_base,
                    alternate_base,
...
def predict(args, output_config, output_utilities):
    ...
    if args.model_arc == "N1":
        from trio.model import Clair3_Trio_Basic
        ...
        elif args.model_cls == "Clair3_Trio_Out3_denovo":
            m = model_path.Clair3_Trio_Out3_denovo(add_indel_length=args.add_indel_length, predict=True)
        else:
...
def main():
    ...
    parser.add_argument('--model_arc', type=str, default="NN",
