...
"${PYTHON} ${CLAIR3_TRIO} CallVarBam_Denovo \
--chkpnt_fn ${MODEL_PATH_C3D}/${DENOVO_PREFIX} \
...
"${PYPY} ${CLAIR3_TRIO} SortVcf_Trio \
   ...
   --output_fn ${OUTPUT_FOLDER}/{1}_c3d.vcf \
   --sampleName {1} \
   --ref_fn ${REFERENCE_FILE_PATH} \
   --tmp_path ${OUTPUT_FOLDER}/tmp \
   --contigs_fn ${TMP_FILE_PATH}/CONTIGS" ::: ${ALL_SAMPLE[@]}

if [ "$( gzip -fdc ${OUTPUT_FOLDER}/${SAMPLE_C}_c3d.vcf.gz | grep -v '#' | wc -l )" -eq 0 ]; then echo "[INFO] Exit in trio & de novo variant calling"; exit 0; fi
...
TRIO_M_OUTPUT_VCF=(
${OUTPUT_FOLDER}/${SAMPLE_C}_c3d.vcf.gz
${OUTPUT_FOLDER}/${SAMPLE_P1}_c3d.vcf.gz
${OUTPUT_FOLDER}/${SAMPLE_P2}_c3d.vcf.gz
)
...
if [ "$( gzip -fdc ${OUTPUT_FOLDER}/${SAMPLE_C}.vcf.gz | grep -v '#' | wc -l )" -eq 0 ]; then echo "[INFO] Exit in variant calling"; exit 0; fi
