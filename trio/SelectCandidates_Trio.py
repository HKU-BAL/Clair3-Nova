...
def SelectCandidates(args):
    ...
    #     with open(os.path.join(candidate_bed,
    ...
    for idx in range(region_num):
        ...
        split_output = [(item - param.flankingBaseNum, item + param.flankingBaseNum + 2) for item in split_output]
        ...
        # output_path = os.path.join(candidate_bed, '{}.{}_{}'.format(contig_name, split_output[0][0], split_output[-1][1]))
        output_path = os.path.join(candidate_bed, '{}.{}_{}'.format(contig_name, idx, region_num))
    ...
    all_full_aln_regions_path = os.path.join(candidate_bed, 'FULL_ALN_FILE_{}'.format(contig_name))
    with open(all_full_aln_regions_path, 'w') as output_file:
        output_file.write('\n'.join(all_full_aln_regions) + '\n')

def main():

    parser = ArgumentParser(description="Select candidates from pipeup")

    parser.add_argument('--candidate_bed', type=str, default=None,
    ...
    parser.add_argument('--split_bed_size', type=int, default=30000,
