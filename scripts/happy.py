...
def Call(args):
    ...
    # print(','.join([str(item) for item in [ap, ar, af1, sp, sr, sf1, idp, idr, idf1]]), file=output_file)
...
def main():
    ...
    parser.add_argument('--oz', type=int, default=1,
                        help="output type, 1 for float, 2 for number")
...
# gzip -fdc happy.vcf.gz | grep -v '#' | grep 'FP' > fp
# gzip -fdc happy.vcf.gz | grep -v '#' | grep 'FN' > fn
# gzip -fdc happy.vcf.gz | grep -v '#' | grep 'FN' > fn
